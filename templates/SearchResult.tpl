    ...
    <div>
        {if $show_index_error eq true}
            <div class="alert alert-error">
              ...
              {vtranslate('Your index is empty. Please configure the module in the VtigerCRM Settings and wait until the Index was fully created the first time.', 'FulltextSearch')}
            </div>
        {/if}
        ...
        {foreach from=$modules item=module}
            ...
            {if !empty($searchResults[$module.module].records)}
                ...
                <table class="table table-bordered listViewEntriesTable" data-module="{$module.module}">
                    ...
                    <tr>
                        <th>&nbsp;</th>
                        <th colspan="{count($module.labels)}">
                            {if !empty($pdftemplates[$module.module])}
                                <div style="width:600px;">
                                    {vtranslate('LBL_PDFMAKER_TEMPLATE', 'PDFMaker')}:
                                    <select id="pdftemplate_{$module.module}" class='pdfmakerTemplate' style="margin:0;">
                                        {foreach from=$pdftemplates[$module.module] key=templateid item=templatename}
                                            <option value="{$templateid}">{$templatename}</option>
                                        {/foreach}
                                    </select>
                                    <input type="button" class="btn btn-primary fulltextPDFMakerLink" value="{vtranslate('LBL_EXPORT', 'FulltextSearch')}"/>
                                    <input type="button" class="btn btn-primary fulltextPDFMakerEditLink" value="{vtranslate('Edit and Export', 'FulltextSearch')}"/>
